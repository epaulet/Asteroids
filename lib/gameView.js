(function () {
  var GameView = Asteroids.GameView = function (game, ctx, scoresheet) {
  ...
  GameView.prototype.renderTitleScreenContents = function () {
    ...
    ctx.textAlign = 'left';
    ctx.fillText("Get hit by an Asteroid: lose life and respawn.", 310, 135);
    ctx.fillText("Destroy an Asteroid: Score points!", 310, 165);
  ...
  GameView.prototype.start = function () {
    ...
    var gameView = this;
    Asteroids.Sounds.spaceTheme();
    var refreshInterval = setInterval(function () {
      gameView.game.step();
      gameView.scoresheet.level.innerHTML = gameView.game.level;
      gameView.scoresheet.lives.innerHTML = gameView.game.ship.lives;
      gameView.scoresheet.points.innerHTML = gameView.game.points;
      gameView.game.draw(gameView.ctx);
      if (gameView.game.over) {
        clearInterval(refreshInterval);
        Asteroids.Sounds.spaceThemeOff();
